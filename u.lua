...
local FARMER_NAME = "metamystic54"
...
local function teleportSelfAndTryBringPartner(placeId, reason)
    ...
    if ok then
        print("[Autofarm] " .. tostring(reason) .. " -> Teleport(" .. tostring(placeId) .. ") (" .. role .. ")")
    else
        warn("[Autofarm] Teleport failed: " .. tostring(err))
    end
end

-- If one role teleports out, the other should follow even if not in the same place/server.
-- In practice, when either client teleports, the other client will see partner missing and trigger this.
task.spawn(function()
    local MISSING_SECONDS = 5
    local CHECK = 0.5
    local DEBOUNCE = 12
    ...
    while player.Parent do
        -- Exclude this watchdog while in the extra-player-check place.
        -- (When both roles are in 18517967096, temporary roster changes shouldn't force Teleport(18474291382).)
        if tonumber(game.PlaceId) == PLACE_ID_EXTRA_PLAYERS_CHECK then
            missingSince = nil
            task.wait(CHECK)
            continue
        end
        -- FARMER stays when SACRIFICE leaves (blocks done or disconnect); don't treat as "partner missing" hop.
        ...
        if not p then
            ...
            if (tick() - missingSince) >= MISSING_SECONDS then
                local now = tick()
                if now - lastTp >= DEBOUNCE then
                    lastTp = now
                    teleportSelfAndTryBringPartner(18474291382, "Partner missing " .. MISSING_SECONDS .. "s")
                end
            end
        else
            missingSince = nil
...
-- ═══════════════════════════════════════════════════════════════════
--                  STUCK IN MAIN PLACE -> SERVER HOP
-- ═══════════════════════════════════════════════════════════════════

task.spawn(function()
    local THRESHOLD_SECONDS = 60
    local CHECK_INTERVAL = 1.0
    local HOP_DEBOUNCE = 12
    local enteredAt = nil
    local lastHop = 0
    ...
    while player.Parent do
        ...
        if inMain then
            ...
            if (tick() - enteredAt) >= THRESHOLD_SECONDS then
                local now = tick()
                if now - lastHop >= HOP_DEBOUNCE then
                    lastHop = now
                    local ok, err = pcall(function()
                        teleportSelfAndTryBringPartner(18474291382, "Main place timeout")
                    end)
                    if ok then
                        print("[Autofarm] Main place " .. tostring(AUTOFARM_PLACE_ID) .. " for " .. THRESHOLD_SECONDS .. "s+ -> Teleport(18474291382) (" .. role .. ")")
                    else
                        warn("[Autofarm] Main-place teleport failed: " .. tostring(err))
                    end
                end
                enteredAt = tick()
            end
...
-- Safety: if we land in this place at any time, return to hub immediately.
task.spawn(function()
    local BAD_PLACE_ID = 18474291382
    local DEBOUNCE = 6
    local lastTp = 0

    local function tpHome(reason)
        local now = tick()
        if now - lastTp < DEBOUNCE then
            return
        end
        lastTp = now
        local ok, err = pcall(function()
            TeleportService:Teleport(AUTOFARM_PLACE_ID, Players.LocalPlayer)
        end)
        if ok then
            print("[Autofarm] " .. tostring(reason) .. " -> Teleport(" .. tostring(AUTOFARM_PLACE_ID) .. ") (" .. role .. ")")
        else
            warn("[Autofarm] Bad-place teleport failed: " .. tostring(err))
        end
    end

    local function check()
        if tonumber(game.PlaceId) == BAD_PLACE_ID then
            tpHome("Bad PlaceId " .. tostring(BAD_PLACE_ID))
        end
    end

    check()
    game:GetPropertyChangedSignal("PlaceId"):Connect(check)
    while player.Parent do
        task.wait(1.0)
        check()
    end
...
-- Watch for disconnect/idle prompts and route through recovery place.
-- Error 273 waits 30s first; other known disconnect errors hop immediately.
task.spawn(function()
    local POPUP_POLL = 0.45
    local HOP_DEBOUNCE = 8
    local ERROR_HOP_PLACE_ID = 18474291382
    local ERROR_273_WAIT_SECONDS = 30
    local lastHopAttempt = 0
    local delayed273Pending = false

    local function classifyPopupText(t)
        if type(t) ~= "string" or t == "" then
            return nil
        end
        local s = t:upper()
        if string.find(s, "ERROR CODE: 273", 1, true) then
            return "273"
        end
        if string.find(s, "ERROR CODE: 288", 1, true) or string.find(s, "ERROR CODE: 278", 1, true) then
            return "IMMEDIATE"
        end
        if string.find(s, "DISCONNECTED", 1, true) and string.find(s, "RECONNECT", 1, true) then
            return "IMMEDIATE"
        end
        if string.find(s, "DISCONNECTED FROM THE EXPERIENCE", 1, true) then
            return "IMMEDIATE"
        end
        if string.find(s, "IDLE 20 MINUTES", 1, true) then
            return "IMMEDIATE"
        end
        return nil
    end

    local function scanGuiContainer(root)
        if not root then
            return nil
        end
        local saw273 = false
        for _, inst in ipairs(root:GetDescendants()) do
            if inst:IsA("TextLabel") or inst:IsA("TextButton") or inst:IsA("TextBox") then
                local mode = classifyPopupText(inst.Text)
        ...
        return saw273 and "273" or nil
    end

    local function getDisconnectPopupMode()
        local mode = nil
        pcall(function()
            local pg = player:FindFirstChild("PlayerGui")
            if pg then
                local pMode = scanGuiContainer(pg)
                if pMode == "IMMEDIATE" then
                    mode = "IMMEDIATE"
                    return
                end
                if pMode == "273" then
            ...
            if core then
                local cMode = scanGuiContainer(core)
                if cMode == "IMMEDIATE" then
                    mode = "IMMEDIATE"
                    return
                end
                if cMode == "273" then
    ...
    local function hopToErrorPlace(reason)
        local now = tick()
        if now - lastHopAttempt < HOP_DEBOUNCE then
            return
        end
        lastHopAttempt = now
        local ok, err = pcall(function()
            TeleportService:Teleport(ERROR_HOP_PLACE_ID, Players.LocalPlayer)
        end)
        if ok then
            print("[Autofarm] " .. tostring(reason) .. " -> Teleport(" .. tostring(ERROR_HOP_PLACE_ID) .. ") (" .. role .. ")")
        else
            warn("[Autofarm] Error-popup teleport failed: " .. tostring(err))
        end
    end

    while player.Parent do
        task.wait(POPUP_POLL)
        local mode = getDisconnectPopupMode()
        if mode == "IMMEDIATE" then
            hopToErrorPlace("Disconnect/idle popup detected")
        elseif mode == "273" and not delayed273Pending then
            delayed273Pending = true
            print("[Autofarm] Error 273 detected -> waiting " .. ERROR_273_WAIT_SECONDS .. "s before Teleport(" .. tostring(ERROR_HOP_PLACE_ID) .. ")")
            task.spawn(function()
                task.wait(ERROR_273_WAIT_SECONDS)
                delayed273Pending = false
                if player.Parent then
                    hopToErrorPlace("Error 273 wait complete")
                end
            end)
        else
            -- Allow future 273 waits only when the popup reappears.
            delayed273Pending = false
        end
    end
end)
...
task.spawn(function()
    ...
    local function tryTeleportHome()
        ...
        if ok then
            print("[Autofarm] Teleport -> main place " .. tostring(AUTOFARM_PLACE_ID) .. " (extra players / wrong roster). LocalPlayer=" .. player.Name)
        else
            warn("[Autofarm] Teleport failed: " .. tostring(err))
        end
    end
...
local didTeleportToFarmer = false
while not (Players:FindFirstChild(config.farmer) and Players:FindFirstChild(config.sacrifice)) do
    if not didTeleportToFarmer and role == "SACRIFICE" and (tick() - coopWaitStart) > 2 then
        didTeleportToFarmer = true
        local friendsRem = RS:FindFirstChild("Remotes")
        friendsRem = friendsRem and friendsRem:FindFirstChild("Friends")
        if friendsRem and (friendsRem:IsA("RemoteEvent") or friendsRem:IsA("UnreliableRemoteEvent")) then
            ...
            print("[Autofarm] Friends: TeleportToPlayer -> " .. config.farmer .. " (co-op wait > 2s)")
        else
...
local function setMoveKey(keyName, isDown)
    if keyMoveState[keyName] ~= isDown then
        ...
        VIM:SendKeyEvent(isDown, Enum.KeyCode[keyName], false, game)
    end
end

local function stopKeyMove()
                    ...
                    while sessionAlive(sid) and player.Parent do
                        ...
                        if txt and shouldHopFromFindingMatchDisplay(txt) then
                            ...
                            if tnow - lastHop >= HOP_DEBOUNCE then
                                ...
                                if ok then
                                    print("[Autofarm] Matchmaking stuck (FINDING MATCH " .. FINDING_MATCH_MIN_SECONDS .. "s+) -> server hop (" .. role .. ")")
                                else
                                    warn("[Autofarm] Matchmaking server hop failed: " .. tostring(err))
                                end
                                return
                            end
                        end
...
local function restartAutofarmPartnerSync()
    ...
    if last and (tick() - last) < 0.75 then
        return
    end
...
Players.PlayerRemoving:Connect(function(p)
    ...
    if p == player or p.Name == config.sacrifice or p.Name == config.farmer then
        state.running = false
        _G.AutofarmRunning = false
        stopMoveTween()
        stopKeyMove()
