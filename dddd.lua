    ...
    SaveManager:SetFolder("pbvanta/config")
        ...
        do -- Shooting
                ...
                Tabs.Shooting:AddSection("Timer")
                    ...
                    -- ============================================
                    -- GLIDE SECTION (Side 1)
                    -- ============================================
                Tabs.Shooting:AddSection("Glide settings")

                    do
                        -- Initialize glide speed
                        getgenv().GlideSpeed = getgenv().GlideSpeed or 5

                        local player = Players.LocalPlayer
                        local charName = player.Name
                        
                        Tabs.Shooting:CreateToggle("GlideToggle", {
                            Title = "Shot Glide",
                            Default = false,
                            Callback = function(Value)
                                if Value then
                                    ...
                                    -- Attribute listener
                                    local attrConnection = obj.AttributeChanged:Connect(function(attributeName)
                                        if attributeName == "Action" then
                                            local value = obj:GetAttribute("Action")
                                            actionActive = (value == "Shooting")
                                        end
                                    end)
                                    
                                    -- Glide movement (Heartbeat: same physics-step feel, less render-thread load than RenderStepped)
                                    local glideConnection = RunService.Heartbeat:Connect(function(deltaTime)
                                        if not actionActive then return end
                                        ...
                                        if moveDirection.Magnitude > 0 then
                                            ...
                                            humanoidRootPart.CFrame = humanoidRootPart.CFrame + 
                                                (moveDirection * (getgenv().GlideSpeed or 5) * deltaTime)
                                        end
                                    end)
                                    ...
                                    -- Store connections
                                    getgenv().GlideConnections = {attrConnection, glideConnection, respawnConnection}
                                else
                                    -- Cleanup connections
                                    if getgenv().GlideConnections then
                                        for _, conn in ipairs(getgenv().GlideConnections) do
                                            if conn then conn:Disconnect() end
                                        end
                                        getgenv().GlideConnections = nil
                                    end
                                end
                            end
                        })
                        
                        Tabs.Shooting:CreateSlider("Shooting_Glide_Amount", {
                            Title = "Glide Amount",
                            Default = getgenv().GlideSpeed,
                            Max = 20,
                            Min = 1,
                            Rounding = 1,
                            Callback = function(Value)
